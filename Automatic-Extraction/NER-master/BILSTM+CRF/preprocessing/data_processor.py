...
def bulid_vocab(vocab_size,
    ...
    with open(os.path.join(config.ROOT_DIR, config.RAW_SOURCE_DATA), 'r',encoding='UTF-8') as fr:
...
def text2id(word2id, maxlen=None, valid_size=0.2, random_state=2018, shuffle=True, is_debug=False):
    ...
    if len(glob(file_name)) > 0:
        logger.info('Text to id file existed')# C:\Users\NR>type test.txt | find /v /c 
        epoch_size = -1
        for epoch_size, line in enumerate(open(file_name, 'rU')):
            pass
        epoch_size += 1
        #poch_size = int(os.popen('type %s | find /v /c\"\"'%file_name).readlines()[0].strip('\n'))
        return epoch_size

    logger.info('Text to id')
    sentences, labels = [], []
    size = 0
    with open(os.path.join(config.ROOT_DIR, config.RAW_SOURCE_DATA), 'r',encoding='UTF-8') as fr_1, \
            open(os.path.join(config.ROOT_DIR, config.RAW_TARGET_DATA), 'r',encoding='UTF-8') as fr_2:
    ...
    with open(config.TRAIN_FILE, 'w',encoding='UTF-8') as fw:
    ...
    with open(config.VALID_FILE, 'w',encoding='UTF-8') as fw:
